  ...
  return new Promise(async function(resolve, reject) {
    return generateOrder(
      ...
      var requestHash = getPayURequestHash({
        ...
        product_info: params.payment_details[0].package_name
          ? params.payment_details[0].package_name
          : params.products[0].assessment_name, //productInfo.name,
    ...
    .then((result) => {
      try {
        return runSqlQueryAsyncSelect(
          ...
          //sendCongratulationMailAssessment(product_name, paymentInfo.email);
          /*sendAdminMail(
            paymentInfo.transactionId,
            paymentInfo.name,
            paymentInfo.email,
...
function sendCongratulationMailAssessment(product_name, email) {
  const data = {
