...
router.post("/DisplayLiveCourses", (req, res) => {
  ...
  if (geo && geo.country != "IN")
    query =
      "SELECT live_course_id,'USD' as 'cprice', live_course_name, live_course_slug,img, live_course_outcomes, grade, age_group, per_hour_price, duration, live_courses.int_actual_price 'actual_price' ,int_course_price 'course_price', course_theme,  orders.status 'purchases_status',products.product_id,products.product_type,(SELECT avg(product_feedback.obt_rat) from product_feedback group by product_feedback.product_id HAVING product_feedback.product_id = live_courses.product_id) 'avg_rating' FROM `live_courses` inner join products on (products.product_id =live_courses.product_id) left join orders on (products.product_id =orders.product_id and user_id=? and orders.status=1)";
  else
    query =
      "SELECT live_course_id,'IN' as 'cprice', live_course_name, live_course_slug,img, live_course_outcomes, grade, age_group, per_hour_price, duration, live_courses.actual_price 'actual_price' ,course_price 'course_price', course_theme,  orders.status 'purchases_status',products.product_id,products.product_type,(SELECT avg(product_feedback.obt_rat) from product_feedback group by product_feedback.product_id HAVING product_feedback.product_id = live_courses.product_id) 'avg_rating' FROM `live_courses` inner join products on (products.product_id =live_courses.product_id) left join orders on (products.product_id =orders.product_id and user_id=? and orders.status=1)";
    ...
    .then((Datarespons) => {
      if (Datarespons.err) throw Datarespons.err;
        ...
        return dbUtils
          ...
          .then((dataResult) => {
            if (dataResult.err) throw dataResult.err;
            else if (params.req_id != undefined && params.req_id != 0)
              return Datarespons;
          ...
          .then(async (assignedResult) => {
            if (assignedResult) {
              if (assignedResult.err) throw assignedResult.err;
                ...
                assigned_id = assignedResult.insertId;

                query =
                  "INSERT INTO `live_class_mail_send`(`mail_send_date`, `username`, `status`, `time`,`platform`,`assigned_id`) VALUES (?,?,?,?,?,?)";
                param = [s_date1, username, 0, s_time, 1, assigned_id];
                return await dbUtils.runSqlQueryAsyncInsert(query, param);
                //console.log("success");
          ...
          .then(async (dataResultForCVMail) => {
            if (dataResultForCVMail) {
              if (dataResultForCVMail.err) throw dataResultForCVMail.err;
                ...
                if (dataResultForCVMail && dataResultForCVMail.result.length) {
...
async function sendScheduleSMS(...details) {
  ...
  await asyncforEach(details, async (detail, index) => {
   ...
   https://test.codevidhya.com/live-class?reqId=${details[0].reqId}
     ...
     <table class="es-content" cellspacing="0" cellpadding="0" align="center" style="mso-table-lspace:0pt;mso-table-rspace:0pt;border-collapse:collapse;border-spacing:0px;table-layout:fixed !important;width:100%;"> 
       <tr style="border-collapse:collapse;"> 
        <td align="center" style="padding:0;Margin:0;"> 
         ...
         <p style="Margin:10;-webkit-text-size-adjust:none;-ms-text-size-adjust:none;mso-line-height-rule:exactly;font-size:14px;font-family:arial, 'helvetica neue', helvetica, sans-serif;line-height:21px;color:#333333;">
        <center><a style="text-decoration:none;padding:8px 16px; background:#008dd2;color:#fff" href="https://test.codevidhya.com/live-class?reqId=${details[0].reqId}" target="_blank">Join Class </a></center>
         </p>
         </td>
         </tr>
         </table>
...
async function remainderSMS(...details) {
  ...
  await asyncforEach(details, async (detail, index) => {
    ...
    A gentle reminder for your demo class scheduled at:
       ...
       Module : ${detail.theme}
   Here is the exclusive link to join your class:
   https://test.codevidhya.com/live-class?reqId=${detail.req_id}
   ...
   <table class="es-content" cellspacing="0" cellpadding="0" align="center" style="mso-table-lspace:0pt;mso-table-rspace:0pt;border-collapse:collapse;border-spacing:0px;table-layout:fixed !important;width:100%;"> 
     <tr style="border-collapse:collapse;"> 
      <td align="center" style="padding:0;Margin:0;"> 
       ...
       <p style="Margin:10;-webkit-text-size-adjust:none;-ms-text-size-adjust:none;mso-line-height-rule:exactly;font-size:14px;font-family:arial, 'helvetica neue', helvetica, sans-serif;line-height:21px;color:#333333;">
         <center><a style="text-decoration:none;padding:8px 16px; background:#008dd2;color:#fff" href="https://test.codevidhya.com/live-class?reqId=${detail.req_id}" target="_blank">Join Class </a></center>
         </p>
       </td>
       </tr>
       </table>
...
router.post("/getTrainerClassSlots", (req, res) => {
  let params = req.body;
  let query =
    "SELECT trainer_calendar.trainer_calender_id,live_demo_course_assigned.assigned_id,live_demo_course_assigned.req_id,trainer_calendar.date_s_time_slot,Date_Format(trainer_calendar.date_s_time_slot,'%W %M %e %Y') 'schedule_date',Date_Format(trainer_calendar.date_s_time_slot,'%h:%i %p') 'schedule_s_time',Date_Format(trainer_calendar.date_e_time_slot,'%h:%i %p') 'schedule_e_time',live_demo_course_assigned.feedback_status_id,live_demo_course_assigned.sale_product_id,feedback_status.feedback_status,live_demo_course_assigned.feedback_message,trainer_calendar.date_e_time_slot,live_demo_course_assigned.cancle_region,(case when live_demo_course_assigned.cancle_region IS NOT NULL then 'cancle' when live_class_mail_send.feedback_mail=1 then 'done' when live_class_mail_send.reschedule=1 then 'reschedule' else 'not_done' END) 'statusss',trainer_calendar.duration,trainer_calendar.break_time,(case when find_in_set(?,trainer_calendar.booked_ids) then '1' else '0' end) 'booked',demo_trial_courses.demo_course_name 'course_name',trainer_class_link.class_link,cv_users.name,cv_users.user_id,cv_classes.cls_name 'grade',live_class_student_feedback.student_feedback_id,live_class_student_feedback.skills,  live_class_student_feedback.courses_ids,live_class_student_feedback.course_suggestion,live_class_student_feedback.comments from trainer_calendar left join live_demo_course_assigned on (live_demo_course_assigned.trainer_calender_id =trainer_calendar.trainer_calender_id and live_demo_course_assigned.trainer_id=?) left join feedback_status on (feedback_status.feedback_status_id = live_demo_course_assigned.feedback_status_id) left join demo_trial_courses on (demo_trial_courses.demo_course_id =live_demo_course_assigned.demo_course_id) left join trainer_class_link on (live_demo_course_assigned.t_class_link_id = trainer_class_link.t_class_link_id) left join cv_users on (cv_users.user_id =live_demo_course_assigned.user_id) left join cv_st_detail on (cv_st_detail.user_id =cv_users.user_id) left join cv_classes on (cv_classes.cls_id = cv_st_detail.cls_id) left join live_class_mail_send on (live_class_mail_send.username =cv_users.username) left join live_class_student_feedback on (live_class_student_feedback.req_id =live_demo_course_assigned.req_id) where find_in_set(?,user_ids) order by trainer_calendar.date_s_time_slot";
          ...
          await asyncforEach(resultData, async (item, index) => {
            ...
            let booked_params =
              params.booked != 0
                ...
                : "";
            query = `SELECT trainer_calendar.trainer_calender_id,'${item.name}' as 'trainer_name','${item.user_id}' as 'trainer_id',live_demo_course_assigned.assigned_id,live_demo_course_assigned.req_id,trainer_calendar.date_s_time_slot,Date_Format(trainer_calendar.date_s_time_slot,'%W %M %e %Y') 'schedule_date',Date_Format(trainer_calendar.date_s_time_slot,'%h:%i %p') 'schedule_s_time',Date_Format(trainer_calendar.date_e_time_slot,'%h:%i %p') 'schedule_e_time',live_demo_course_assigned.feedback_status_id,live_demo_course_assigned.sale_product_id,feedback_status.feedback_status,live_demo_course_assigned.feedback_message,trainer_calendar.date_e_time_slot,live_demo_course_assigned.cancle_region,(case when live_demo_course_assigned.cancle_region IS NOT NULL then 'cancle' when live_class_mail_send.feedback_mail=1 then 'done' when live_class_mail_send.reschedule=1 then 'reschedule' else 'not_done' END) 'statusss',trainer_calendar.duration,trainer_calendar.break_time,(case when find_in_set(${item.user_id},trainer_calendar.booked_ids) then '1' else '0' end) 'booked',demo_trial_courses.demo_course_name 'course_name',trainer_class_link.class_link,cv_users.name,cv_users.user_id,cv_classes.cls_name 'grade',live_class_student_feedback.student_feedback_id,live_class_student_feedback.skills,  live_class_student_feedback.courses_ids,live_class_student_feedback.course_suggestion,live_class_student_feedback.comments from trainer_calendar ${booked} left join feedback_status on (feedback_status.feedback_status_id = live_demo_course_assigned.feedback_status_id) left join demo_trial_courses on (demo_trial_courses.demo_course_id =live_demo_course_assigned.demo_course_id) left join trainer_class_link on (live_demo_course_assigned.t_class_link_id = trainer_class_link.t_class_link_id) left join cv_users on (cv_users.user_id =live_demo_course_assigned.user_id) left join cv_st_detail on (cv_st_detail.user_id =cv_users.user_id) left join cv_classes on (cv_classes.cls_id = cv_st_detail.cls_id) left join live_class_mail_send on (live_class_mail_send.username =cv_users.username) left join live_class_student_feedback on (live_class_student_feedback.req_id =live_demo_course_assigned.req_id) where find_in_set(${item.user_id},user_ids) ${courseId} ${selval} ${booked_params} order by date_s_time_slot`;
            param = [];
            // console.log(query);
            await dbUtils
              .runSqlQueryAsyncSelect(query, param)
  ...
  let notBooked =
    params.booked != 0
      ? params.booked == "not_booked"
        ...
        : ` and (trainer_calendar.trainer_calender_id IN(SELECT live_demo_course_assigned.trainer_calender_id from live_demo_course_assigned where live_demo_course_assigned.trainer_id=${params.user_id}) or find_in_set(${params.user_id},booked_ids))`
      : " ";
  if (params.selval != 0) {
    let start_date = new Date(params.start_date);
    start_date = dateFormat(start_date, "yyyy-mm-dd");
...
router.post("/postCancleRegion", (req, res) => {
  let params = req.body;
  console.log(params);
  let query =
    "UPDATE `live_demo_course_assigned` SET live_demo_course_assigned.cancle_region=?,live_demo_course_assigned.cancle_by=? where live_demo_course_assigned.assigned_id=?";
  return dbUtils
    .runSqlQueryAsyncUpdate(query, [
      params.detail,
      params.user_id,
      params.assigned_id
    ])
    .then((dataResult) => {
      if (dataResult.err) throw dataResult.err;
        ...
        //information set CV To LS by creating Function and remove This lead from cv
        return res.send(dataResult.result);
      }
    });
});
    ...
    .then((result) => {
      if (result.err) {
        ...
        if (result.result[0].trainer == 1) {
          ...
          query =
            "SELECT `live_course_id`, `live_course_name`, `live_course_slug`,`img`, `live_course_outcomes`, `grade`, `age_group`, `per_hour_price`, `duration`, `course_price`, `course_theme`,description,  orders.status 'purchases_status',products.product_type,(SELECT avg(product_feedback.obt_rat) from product_feedback group by product_feedback.product_id HAVING product_feedback.product_id = live_courses.product_id) 'avg_rating' FROM `live_courses` inner join products on (products.product_id =live_courses.product_id) inner join orders on (products.product_id =orders.product_id and user_id=? and orders.status=1)";
        }
        return dbUtils
          .runSqlQueryAsyncSelect(query, [params.user_id])
          .then((data) => {
...
router.post("/OtherCourses", (req, res) => {
 ...
 //let ip = "117.220.166.122"; //"207.97.227.239"; //req.headers["x-forwarded-for"] || req.connection.remoteAddress;
  ...
  //"SELECT `live_course_id`, `live_course_name`, `live_course_slug`,`img`, `live_course_outcomes`, `grade`, `age_group`, `per_hour_price`, `duration`, `course_price`, `course_theme`,  orders.status 'purchases_status',products.product_type,(SELECT avg(product_feedback.obt_rat) from product_feedback group by product_feedback.product_id HAVING product_feedback.product_id = live_courses.product_id) 'avg_rating' FROM `live_courses` inner join products on (products.product_id =live_courses.product_id) left join orders on (products.product_id =orders.product_id and user_id=? and orders.status=1) WHERE live_courses.live_course_slug!=?",
  return dbUtils
    .runSqlQueryAsyncSelect(query, [params.user_id, params.courseSlug])
    .then((result) => {
      if (result.err) {
        throw result.err;
      } else {
...
router.post("/getCourseDetails", (req, res) => {
  ...
  if (geo && geo.country != "IN")
    query =
      "SELECT `live_course_id`,'USD' as 'cprice' ,`live_course_name`,live_courses.description, `live_course_slug`,`img`,live_courses.requirement,live_courses.course_includes, `live_course_outcomes`, `live_course_outcomes`, `grade`, `age_group`, `per_hour_price`, `duration`,live_courses.int_actual_price 'actual_price', int_course_price 'course_price', `course_theme`,live_courses.product_id,  orders.status 'purchases_status',products.product_type,(SELECT avg(product_feedback.obt_rat) from product_feedback group by product_feedback.product_id HAVING product_feedback.product_id = live_courses.product_id) 'avg_rating' FROM `live_courses` inner join products on (products.product_id =live_courses.product_id) left join orders on (products.product_id =orders.product_id and user_id=? and orders.status=1) where live_courses.live_course_slug=?";
  else
    query =
      "SELECT `live_course_id`,'IN' as 'cprice', `live_course_name`,live_courses.description, `live_course_slug`,`img`,live_courses.requirement,live_courses.course_includes, `live_course_outcomes`, `live_course_outcomes`, `grade`, `age_group`, `per_hour_price`, `duration`,live_courses.actual_price, `course_price`, `course_theme`,live_courses.product_id,  orders.status 'purchases_status',products.product_type,(SELECT avg(product_feedback.obt_rat) from product_feedback group by product_feedback.product_id HAVING product_feedback.product_id = live_courses.product_id) 'avg_rating' FROM `live_courses` inner join products on (products.product_id =live_courses.product_id) left join orders on (products.product_id =orders.product_id and user_id=? and orders.status=1) where live_courses.live_course_slug=?";
  return dbUtils
    .runSqlQueryAsyncSelect(query, [params.user_id, params.courseSlug])
    .then((result) => {
      if (result.err) {
  ...
  return await dbUtils
    ...
    .then(async (dataRes) => {
      if (dataRes.err) throw dataRes.err;
        ...
        let course1 = `https://test.codevidhya.com/assets/psycometric_icons/${courseSuggestion[0]}.png`;
        let course2 = `https://test.codevidhya.com/assets/psycometric_icons/${courseSuggestion[1]}.png`;
        console.log(course1 + " " + course2);
        await res.render(
          "studentReport",
          {
        ...
        await asyncforEach(items, async (item, index1) => {
          ...
          let fetchCourseId =
            item["Demo Courses"] != undefined
              ? `(select demo_course_id from demo_trial_courses where demo_course_name='${item["Demo Courses"]}')`
              : "0";
              ...
              if (dataResult.err) throw dataResult.err;
                ...
                let salePerson_id =
                  item && item["Alloted To(Sales)"] != undefined
                    ? `(select user_id from cv_users where name ='${item["Alloted To(Sales)"]}' and sch_id in (SELECT sch_id FROM cv_school_detail where cv_school_detail.cv_pid=4))`
                    : " 0";
                ...
                let grade =
                  item && item["Grade"] != undefined
                    ? `(select cls_id from cv_classes where cls_name ='${item["Grade"]}')`
                    : `0`;
                if (scheduleDate) {
              ...
              if (RecordResponse) {
                if (RecordResponse.err) throw RecordResponse.err;
                  ...
                  let salePerson_email =
                    item && item["Alloted To(Sales)"] != undefined
                      ? `(select * from cv_users where name ='${item["Alloted To(Sales)"]}' and sch_id in (SELECT sch_id FROM cv_school_detail where cv_school_detail.cv_pid=4))`
                      : " 0";
...
router.post("/admin_getAllUnscheduleSalesLead", (req, res) => {
  ...
  (case when cv_users.name IS NOT NULL then cv_users.name else 'codevidhya' end) 'sales_person_name'
...
router.get("/getCoupon/:coupon_code", async (req, res) => {
  try {
    const code = req.params.coupon_code;
    const user_id = req.body.user_id;
    let query =
      "SELECT `coupon_id`, `discount`, `user_id`, `times_used` FROM `cv_coupons` WHERE coupon_code=?";
    let param = [code];
    const data = await dbUtils.runSqlQueryAsyncSelect(query, param);
    console.log(data.result[0]);
    if (data.result[0].user_id !== 0) {
      let query =
        "SELECT COUNT(id) FROM `orders` WHERE status = 1 AND coupon_id = ?";
      param = [data.result[0].coupon_id];
      let count = await dbUtils.runSqlQueryAsyncSelect(query, param);
      if (count.result[0]["COUNT(id)"] < data.result[0]["times_used"]) {
        return res.send({
      ...
      let query =
        "SELECT COUNT(id) FROM `orders` WHERE status = 1 AND coupon_id = ? AND user_id = ?";
      param = [data.result[0].coupon_id, user_id];
      let count = await dbUtils.runSqlQueryAsyncSelect(query, param);
      if (count.result[0]["COUNT(id)"] < data.result[0]["times_used"]) {
        return res.send({
          discount: data.result[0].discount
