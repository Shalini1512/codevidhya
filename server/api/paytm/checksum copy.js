...
function paramsToString(params, mandatoryflag) {
  ...
  var tempValues =Object.values(params);
 
  tempKeys.sort();
  
  tempKeys.forEach((key,index)=> {
  let compareValue = (params[key]).toString();
  var n = compareValue.includes("REFUND"); //params[key].includes("REFUND"); 
  console.log(n);
   var m = compareValue.includes("|"); //params[key].includes("|")  
        if(n == true )
        {
          params[key] = "";
        }
