...
ActiveCode.prototype.createEditor = function (index) {
        ...
        $(this.containerDiv).addClass("container-fluid");
        $(this.containerDiv).css("background","#0d2d62");
		//$(this.containerDiv).css("border","thin solid red");
        $(this.containerDiv).css("display","flex");
        $(this.containerDiv).css("flex-direction","column");
        $(this.containerDiv).css("height","100%");
        $(this.containerDiv).css("padding","0");
        $(this.containerDiv).css("margin","0");
		//$(this.containerDiv).css("position","absolute");
		//$(this.containerDiv).css("top","0px");
		
		//$(this.containerDiv).css("z-index","9000");
    ...
    $(workSpaceDiv).addClass("col-sm-12");
    $(workSpaceDiv).css("padding","0");
    $(workSpaceDiv).css("flex-grow","1");
    
    this.workSpaceDiv = workSpaceDiv;
	
    $(codeDiv).addClass("ac_code_div col-sm-6");
    $(codeDiv).css("display","flex");
    $(codeDiv).css("flex-direction","column");
    $(codeDiv).css("padding","0");
	$(codeDiv).css("height","100%");
    $(codeDiv).attr('id', 'code_div');
    // header for editor 
    var leftheaderDiv = document.createElement("div");
    $(leftheaderDiv).addClass("ac_leftheader");
    $(leftheaderDiv).addClass("col-sm-12 bg-info");
    $(leftheaderDiv).text("main.py");
    //end
    $(codeDiv).prepend(leftheaderDiv);
    ...
    this.workSpaceDiv.appendChild(codeDiv);

    this.containerDiv.appendChild(workSpaceDiv);
...
ActiveCode.prototype.setEditorValue = function (){
    if(this.crntexpno >=this.fstexpno && this.crntexpno <= this.lstexpno){
        this.editor.setValue(this.quickExamples[this.crntexpno].content)
        this.runProg();
    }
};
...
ActiveCode.prototype.loaddata = function() {
    var obj=this;
    $.post("/api/user/QuickExamples",{ role_id:'3', cls_id:'8' }, function(data){
        obj.quickExamples = data.quickExamples;
        obj.fstexpno = 0
        obj.crntexpno = 0
        obj.lstexpno = obj.quickExamples.length -1 
        obj.setEditorValue()
    });
};
...
ActiveCode.prototype.createControls = function () {
    ...
    $(ctrlDiv).addClass("col-sm-12");
    $(ctrlDiv).css("padding","10px 10px");
    //back to web editor
    var butt = document.createElement("button");
    $(butt).text("Back");
	$(butt).attr('id', 'backToWebEditor');
	...
	if (! this.hidecode) {
        ...
        $(butt).text("Previous");
        $(butt).addClass("btn btn-info");
		$(butt).attr('id', 'pre_btn');
		$(butt).css("margin","5px");
        ctrlDiv.appendChild(butt);
        this.histButton = butt;
       $(butt).click(this.pre_exp.bind(this));
    }
	if (! this.hidecode) {
        var butt = document.createElement("button");
        $(butt).text("Next");
        $(butt).addClass("btn btn-info");
		$(butt).attr('id', 'nxt_btn');
		
		$(butt).css("margin","5px");
        ctrlDiv.appendChild(butt);
        this.histButton = butt;
       $(butt).click(this.nxt_exp.bind(this));
    }
	
	if (! this.hidecode) {
	   var array = ["Select Theme","default","3024-day","3024-night","abcdef","ambiance","base16-dark","base16-light","bespin","blackboard","cobalt","colorforth","dracula","eclipse","elegant","erlang-dark","hopscotch","icecoder","isotope","lesser-dark","liquibyte","material","mbo","mdn-like","midnight","monokai","neat","neo","night","panda-syntax","paraiso-dark","paraiso-light","pastel-on-dark","railscasts","rubyblue","seti","solarized dark","solarized light","the-matrix","tomorrow-night-bright","tomorrow-night-eighties","ttcn","twilight","vibrant-ink","xq-dark","xq-light","yeti","zenburn"];
		//Create and append select list
		var selectList = document.createElement("select");
        $(selectList).addClass("form-control");
        $(selectList).css("background","#ffa22b");
        $(selectList).css("color","#FFFFFF");
        $(selectList).css("width","auto");
        $(selectList).css("margin","5px 5px ");
        $(selectList).css("display","inline-block");
		selectList.id = "mytheme";
		ctrlDiv.appendChild(selectList);
		...
		this.histButton = selectList;
       $(selectList).change(this.apply_theme.bind(this));
    }
	
	if (! this.hidecode) {
	   var array = ["small","medium","large","x-large","xx-large"];
		//Create and append select list
		var selectList = document.createElement("select");
        $(selectList).addClass("form-control");
        $(selectList).css("background","#ffa22b");
		...
		this.histButton = selectList;
       $(selectList).change(this.apply_size.bind(this));
    }
	...
	/*if (! this.hidecode) {
		...
		$(butt).attr('id', 'srch_btn');
		
		$(butt).css("margin","5px");
		$(butt).css("visibility","hidden");
        ctrlDiv.appendChild(butt);
        this.histButton = butt;
       $(butt).click(this.search_exp.bind(this));
    }*/
...
ActiveCode.prototype.createOutput = function () {
    ...
    $(outDiv).addClass("ac_output col-sm-6");
    $(outDiv).css("display","flex");
    $(outDiv).css("flex-direction","column");
    $(outDiv).css("height","100%");
    $(outDiv).css("padding","0");

    var rightheaderDiv = document.createElement("div");
    $(rightheaderDiv).addClass("ac_rightheader");
    $(rightheaderDiv).addClass("col-sm-12 bg-info");
    $(rightheaderDiv).text("Output");
    
    $(outDiv).prepend(rightheaderDiv);

    this.outDiv = outDiv;
    this.output = document.createElement('pre');
	
    $(this.output).addClass("form-control");
    $(this.output).css("border-radius","0");
    $(this.output).css("flex-grow","1");
    $(this.output).css("padding","5px");
    $(this.output).css("margin","0");
    $(this.output).css("height","100%");
    $(this.output).css("visibility","visible");
    
    //$(this.output).css("min-height","620px");
	 //$(this.output).css("height","auto");
	 $(this.output).attr('id', 'output_div');
    ...
    this.workSpaceDiv.appendChild(outDiv);
...
ActiveCode.prototype.showCodelens = function () {
    ...
    var embedUrlStr = $.param.fragment(srcURL, myVars, 2 );
...
ActiveCode.prototype.addErrorMessage = function (err) {
	...
	//$(codeDiv).addClass("ac_code_div col-sm-6");
	//$(codeDiv).css("background","red");
	$(this.eContainer).css("position","absolute");
	$(this.eContainer).css("z-index","9999");
	$(this.eContainer).css("top","20%");
	$(this.eContainer).css("left","25%");
	var butt = document.createElement("button");
        $(butt).text("X");
        $(butt).addClass("btn btn-danger button");
		$(butt).css("border-radius","20%");
		$(butt).css("float","right");
		 $(butt).click(this.close_errDiv.bind(this));
	
	this.eContainer.appendChild(butt);
...
ActiveCode.prototype.close_errDiv = function() {

$("#codeexample1_errinfo").hide();
}
...
errorText.NameError = "A name error almost always means that you have used a variable before it has a value.  Often this may be a simple type, so check the spelling carefully.";
...
ActiveCode.prototype.apply_theme = function() {
	mytheme=$('#mytheme').val();
	this.editor.setOption("theme", mytheme);
	
};
ActiveCode.prototype.apply_size = function() {
	
	mysize=$('#mysize').val();
	$("#code_div").css("font-size",mysize);
	$("#output_div").css("font-size",mysize);
  ...
  //press enter on text area..
 	$('#search-field').keypress(function (e) {
 		...
 		if(key == 13)  // the enter key code
  		{
    		var search_qr=$(this).val();
			$.post("control_data/filterdata.php","search_qr="+search_qr, function(data){
			qr=$(data).filter('#search_qr').html();
			if(qr!=0)
			{
				c_id=$(data).filter('#c_id').html();
				$('#crnt_exp').val(c_id);
				$("#srch_btn").click();
			}
			else
			{
				notify("Sorry! invalid QR code.");
			}
			});
    		return false;  
  		}
...
ActiveCode.prototype.search_exp = function() {
	var obj=this;
	crnt_exp=$('#crnt_exp').val();
	$.post("control_data/filterdata.php","search_qr="+crnt_exp, function(data){
	py_code=$(data).filter('#py_code_div').html();
		p_id=$(data).filter('#p_id').html();
		c_id=$(data).filter('#c_id').html();
		n_id=$(data).filter('#n_id').html();
		$('#pre_exp').val(p_id);
		$('#crnt_exp').val(c_id);
		$('#nxt_exp').val(n_id);
		obj.editor.setValue(py_code);
		$("#run_btn").click();
...
ActiveCode.prototype.nxt_exp = function() {
	if(this.crntexpno < this.lstexpno){
        this.crntexpno = this.crntexpno + 1;
        this.setEditorValue() 
    }
    else{
        this.crntexpno = this.lstexpno
        alert("This is last example.")
    }
};

ActiveCode.prototype.pre_exp = function() {
	if(this.crntexpno >  this.fstexpno){
        this.crntexpno = this.crntexpno - 1; 
        this.setEditorValue()
    }
    else{
...
ActiveCode.prototype.runProg = function() {
	  ...
	  var saveCode;
        ...
        //$(this.codeDiv).switchClass("col-md-6","col-sm-6",{duration:500,queue:false});
