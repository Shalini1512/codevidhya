...
export default {
    ...
    cvAuth.getUserId(
      function(userId) {
        ...
        if (this.$route.query.assessmentDetail) {
          this.purchaseType = this.$route.query.purchaseType;
          var keys = Object.keys(this.$route.query.assessmentDetail);
          for (var i = 0; i < keys.length; i++) {
            this.selectedProducts[keys[i]] = this.$route.query.assessmentDetail[
              keys[i]
            ];
          }
        }
  ...
  methods: {
          ...
          for (var i = 0; i < allPackages.length; i++) {
            if (allPackages[i].used_for == "assessment") {
              ...
              if (this.allPackages[i].package_id == this.purchaseType) {
                this.itemInCart.push(this.allPackages[i]);
              }
            }
          }
        }
        ...
        .then(function(res) {
          if (res.body.status == "403") {
            ...
            for (var i = 0; i < assessments.length; i++) {
              if (
                this.selectedProducts.hasOwnProperty(
    ...
    calculatePayblePrice: function() {
      ...
      /*if (this.purchaseType == "Silver") {
        this.payment_details.push({
          subTotal: this.assessments[0].price,
          discountPrice: 0,
          totalPrice: this.assessments[0].price,
          name: this.user_full_name,
          email: this.user_email,
          contact: this.user_contact
        });
        ...
        for (var i = 0; i < this.assessments.length; i++) {
          subTotal = subTotal + parseInt(this.assessments[i].price);
        }
        if (this.purchaseType == "Gold") {
          discountPrice = Math.ceil((subTotal * 19.96) / 100);
        } else if (this.purchaseType == "Platinum") {
          discountPrice = Math.ceil((subTotal * 26.61) / 100);
        }
        this.payment_details.push({
          subTotal: subTotal,
