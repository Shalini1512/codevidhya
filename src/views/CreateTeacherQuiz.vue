<template>
  <div id="cQuizPage" class="h-100 w-100" style="position:fixed;">
    ...
    <div class="row h-100 p-0" id="main">
      ...
      >
        <template v-show="quizID ? quiz_detail[0].public_permission == 0 : 0">
          <button
          ...
          <button
            ...
            disabled
          >
            <i class="fas fa-school mx-1"></i>Published
          </button>
          <button
            class="btn btn-success mx-1"
            id="publish"
            ...
            v-else
          >
            <i class="fas fa-school mx-1"></i>Publish For Institute
          </button>
        </template>
      </div>
      <div
      ...
      >
        ...
        >
          <template v-if="quizInitialized">
            <h3>Quiz name</h3>
            <div id="quiz_update_name" v-html="quiz_name"></div>
            <h3>Subject</h3>
    ...
    >
      <div style="padding: 24px 24px 8px;">
        <div class="cv-input-group">
          <label>Subject</label>
          <select id="quiz_subject" class="form-control">
            <option value="0">--select subject--</option>
            <option
              v-for="(subject, sub_index) in subjects"
              :key="sub_index"
...
export default {
  ...
  beforeMount() {
    cvAuth.getUserId(
      function(userId) {
        this.userId = userId;
        this.quiz_type(userId);
        this.get_subject();
        this.get_quiz_question(userId);
      }.bind(this)
    );
  },
  methods: {
    publish_for_school: function(event) {
      ...
      axios
        .post("/api/user/update_inst_permission", {
    ...
    update_quiz_info: function(e) {
      ...
      if (action == "Edit") {
        ...
        axios
          .post("/api/user/update_quiz_basic", {
            quiz_id: this.quiz_id,
            user_id: this.userId,
            quiz_name: quiz_name,
            quiz_desc: quiz_desc
    ...
    next_step: function() {
      ...
      if (
        !(
          this.$store.getters.getAuthData.auth_role_id == 2 ||
          this.$store.getters.getAuthData.auth_role_id == 1
        )
      ) {
        insertUsertypePost = "InsertUserQuizDetail";
      } else {
        insertUsertypePost = "InsertTeacherQuizDetail";
      }
      axios
        .post("/api/user/" + insertUsertypePost, {
        ...
        .then(res => {
          ...
          if (resp == "0") {
            ...
            this.quiz_detail = res.data.data;
            // console.log(this.quiz_detail);
            this.quiz_id = res.data.data[0].quiz_id;
            switch (this.sub_id) {
              case "1":
                this.subjectName = "HTML";
                break;
              case "2":
                this.subjectName = "CSS";
                break;
              case "3":
                this.subjectName = "JavaScript";
                break;
              case "4":
                this.subjectName = "Python";
                break;
              case "5":
                this.subjectName = "SQL";
                break;
              default:
                this.subjectName = "";
            }
    ...
    icon_update: function(e) {
      if (e == "delete") {
        axios
          .post("/api/user/delete_quiz_icon", {
            quiz_id: this.quiz_id,
            user_id: this.userId
          })
          .then(res => {
        ...
        if (file) {
          ...
          this.quiz_img = file.name;
          axios
            .post("/api/user/update_quiz_icon", {
              quiz_img: this.quiz_img,
              quiz_id: this.quiz_id,
              user_id: this.userId
            })
            .then(() => {});
