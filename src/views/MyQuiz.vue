<template>
  ...
  <div id="quiz-admin-root">
    ...
    <!--end aggrement modal-->
    <!--Quiz Publish For Institute-->
    <div id="institute_quiz_guidlines" class="cv-modal medium">
      <div style="padding: 24px 24px 8px;">
        <div class="header text-center" style="width:100%;">
          <h3>
            Guidelines for the teacher to publish this quiz for Institute.
          </h3>
        </div>
        <div class="body">
          <div style="display:flex;flex-direction:row">
            <ol>
              <li>
                Your Quiz can be attempted by users from your institute.
              </li>
              <li>
                You will earn coins depending upon the number of unique users
                who attempt your quiz.
              </li>
            </ol>
          </div>
        </div>
        <div class="p-2 text-right">
          <button
            ...
            onclick="hideModal('institute_quiz_guidlines')"
          >
            Okay
          </button>
        </div>
          ...
          <div style="display:flex;flex-direction:row">
            <ol>
              ...
              <li>
                ...
                unique users attempt your quiz.
              </li>
              <li>
                The additional coins you earn will be 5% of the total number of
                unique users who that attempt the quiz.
              </li>
              <li>Your Quiz can be shared or challenged on social media.</li>
            </ol>
          </div>
        </div>
        <div class="p-2 text-right">
    ...
    <!--end guidlines-->
    <!--Teacger guidlines-->
    <div class="cv-modal medium" id="user_quiz_guidlines">
      <div style="padding: 24px 24px 8px;">
        <div class="header text-center" style="width:100%;">
          <h3>Guidelines for the user to publish this quiz for everyone.</h3>
        </div>
        <div class="body">
          <div style="display:flex;flex-direction:row">
            <ol>
              ...
              <li>
                The coins you earn will be 5% of the total number of unique
                users who that attempt the quiz.
              </li>
              <li>Your Quiz can be shared or challenged on social media.</li>
            </ol>
          </div>
        ...
        <div class="p-2 text-right">
          <button
            class="btn btn-danger"
            onclick="hideModal('user_quiz_guidlines')"
          >
            Okay
          </button>
        </div>
    ...
    <div id="main">
      <div id="quizzes-and-questions-tabs" class="cv-tablayout cardlayout">
        <div class="tabs">
          ...
          <button class="btn btn-success" @click="$router.push('/create-quiz')">
            Create new quiz
          </button>
        </div>
        <!--mcq question-->
          ...
          <div id="quizzes-tablayout" class="cv-tablayout">
            ...
            <template v-for="(quiz_for_sub, sub_index) in subjects">
              ...
              >
                <template
                  v-if="
                    quizzes.findIndex(x => x.sub_id === sub_index + 1) != -1
                  "
                >
                  <template v-for="(quiz, quiz_index) in quizzes">
                    <div
                      v-bind:key="quiz_index"
                      v-if="quiz.sub_id == sub_index + 1"
                    >
                        ...
                        <p v-if="quiz.public_permission == 0">
                          <span class="text-info" style="font-family:'Rubik'"
                            ><b
                              ...
                              ></i
                              >&nbsp;{{
                                quiz.public_institute == 1
                                  ? "Publish this quiz for everyone to earn more coins."
                                  : "Publish this quiz for your institution to earn coins."
                              }}</b
                            ></span
                          >
                        </p>
                      ...
                      <div>
                        ...
                        <!--publish Request-->
                        <template v-if="quiz.public_institute == 1">
                          <button
                          ...
                          >
                          <span
                            v-else-if="quiz.public_permission == 0"
                            class="text-success"
                            style="cursor:pointer"
                            onclick="showModal('user_quiz_guidlines')"
                            ><u>Guidelines </u></span
                          >
                          <div
                            v-if="
                        ...
                        <template v-else>
                          <button
                            ...
                            style="background:#13c047"
                            @click="
                              show_inst_public(quiz.quiz_id, quiz.quiz_name)
                            "
                          >
                            <i class="fas fa-school mx-1"></i>Publish For
                            Institute
                          </button>
                          <span
                            class="text-success"
                            style="cursor:pointer"
                            onclick="showModal('institute_quiz_guidlines')"
                            ><u>Guidelines </u></span
                          >
                        </template>
                        <!--End publish Request-->
               ...
               <template  v-if="quizzes.findIndex(x => x.sub_id ===2)!=-1">
                ...
                <div v-bind:key="q_ind"  v-if="quiz.sub_id==2">
                 ...
                 <div>
                  ...
                  <p>{{quiz.total_question ? quiz.total_question:0}} Questions <span class="spacer"></span>{{quiz.total_marks? quiz.total_marks:0 }} Points <span class="spacer"></span> <span  class="text-success" style="font-family:'Rubik'"><b><i class="fa fa-info-circle" aria-hidden="true"></i> {{quiz.total_attempted? quiz.total_attempted:0}} users attempted</b></span> </p>
                  ...
                  <span  class="mx-2" style="cursor:pointer" @click="quiz_reviews(quiz.quiz_id)">View all</span></span>
                  <p v-if="quiz.public_permission==0"><span  class="text-info" style="font-family:'Rubik'"><b><i class="fa fa-info-circle" aria-hidden="true"></i>&nbsp;Publish this quiz for everyone to earn more coins.</b></span></p>
                </div>
                <i class="flex-filler"></i>
                <div>
                 <button class="btn btn-purple" @click="$router.push({name:'UpdateTeacherQuiz', params:{quiz_slug:quiz.slug,quizID:quiz.quiz_id,SubjectName:quiz.Subject,QuizName:quiz.quiz_name,subID:quiz.sub_id,quizPrice:quiz.price}})">Edit</button>
                 <button v-if="quiz.public_permission==0" class="btn btn-success" style="background:#13c047" @click="show_public(quiz.quiz_id,quiz.quiz_name)">Publish for everyone</button>
                 <button v-if="quiz.public_permission==1&&quiz.visible==0" class="btn btn-gray" style="background:#666;">Requested</button>
                 <span v-if="quiz.public_permission==0" class="text-success" style="cursor:pointer" onclick="showModal('quiz_guidlines')"><u>Guidelines </u></span>
                 <div v-if="quiz.visible==1&&quiz.public_permission==1">
                  <span class="text-success">Approved<i class="fa fa-check" style="color:#28a745;font-size:20px;padding-left:15px;"></i></span>
                 </div>
            ...
            <div v-else style="padding:24px;color:#ccc;text-align:center">There are no Quizzes!</div>
              ...
              <div class="content">
               <template  v-if="quizzes.findIndex(x => x.sub_id ===6)!=-1">
                 <template v-for="(quiz,qz_ind) in quizzes">
                   <div v-bind:key="qz_ind">
                <div  v-if="quiz.sub_id==6">
                <div>
                  <h3>{{quiz.quiz_name}}<span class="subject-tag">{{quiz.Subject}}</span></h3>
                  <p>{{quiz.total_question ? quiz.total_question:0}} Questions <span class="spacer"></span>{{quiz.total_marks? quiz.total_marks:0 }} Points <span class="spacer"></span> <span  class="text-success" style="font-family:'Rubik'"><b><i class="fa fa-info-circle" aria-hidden="true"></i> {{quiz.total_attempted? quiz.total_attempted:0}} users attempted</b></span> </p>
                      <span v-if="quiz.rating" class="mt-1" style="display:flex;flex-direction:row">
...
export default {
  ...
  methods: {
    ...
    submit_public_request: function() {
      axios
        ...
        .then(res => {
          ...
          window.cvNotify("Request sent successfully", "success");
    ...
    show_inst_public: function(quiz_id, quiz_name) {
      axios
        .post("/api/user/sent_inst_request", {
          quiz_id: quiz_id,
          user_id: this.userId
        })
        .then(res => {
          window.cvNotify("Updated Successfully");
          this.get_subject(this.userId);
        });
    },
    ...
    get_subject: function(userId) {
      /* if(!((this.$store.getters.getAuthData.auth_role_id==2)||(this.$store.getters.getAuthData.auth_role_id==1)))
      {
